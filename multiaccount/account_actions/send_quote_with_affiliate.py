...
CONFIG = {
    ...
    "affiliate": "0xffE2C25404525D2D4351D75177B92F18D9DaF4Af"
}

class MultiAccountTradeClient:
    ...
    def fetch_upnl_sig(self, symbol_id: int) -> Tuple:
        ...
        url_params = {
            ...
            "params[partyA]": self.config["sub_account_address"],  # Use sub-account address
    ...
    def send_quote_with_affiliate_via_multiaccount(self) -> int:
        """Execute sendQuoteWithAffiliate function via multiaccount _call"""
        try:
            # 1. Fetch market info
            ...
            # 8. Encode the sendQuoteWithAffiliate function call
            send_quote_txn = self.diamond.functions.sendQuoteWithAffiliate(
                party_bs_white_list,
                market["id"],
                self.config["position_type"],
                ...
                max_funding_rate,
                deadline,
                Web3.to_checksum_address(self.config["affiliate"]),
                upnl_sig
            ).build_transaction({
            ...
            encoded_send_quote = send_quote_txn["data"]

            # 9. Build and send _call transaction
...
def main():
    ...
    print("Quote creation process completed via multiaccount.")
