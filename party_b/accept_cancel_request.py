...
class AcceptCancelRequestClient:
    ...
    def accept_cancel_request(self, quote_id: int):
        """Accept a cancel request for a quote by its ID"""
        try:
            # Build transaction
            txn = self.diamond.functions.acceptCancelRequest(quote_id).build_transaction({
            ...
            print(f"Error accepting cancel request: {e}")
            raise

def main():
    """Main function to demonstrate accepting a cancel request"""
